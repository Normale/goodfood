...
class NutritionVerifierAgent:
    ...
    def verify(self, ingredients: List[str], estimates: Dict[str, Any]) -> Dict[str, Any]:
...
{{
    ...
    "approval_percentage": <0-100, percentage of estimates that seem correct>
        ...
        try:
            ...
            status_color = "green" if approval_pct >= 80 else "yellow" if approval_pct >= 60 else "red"

            issues_text = ""
            if issues_found:
                issues_text = "\n\n[bold]Issues found:[/bold]\n"
                for idx, issue in enumerate(issues_found[:8], 1):
                    severity_color = "red" if issue.get('severity') == 'high' else "yellow" if issue.get('severity') == 'medium' else "blue"
...
def estimate_dish_nutrition(ingredients: List[str], max_iterations: int = 5, approval_threshold: int = 80):
    """
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        console.print(f"[bold white]ITERATION {iteration}/{max_iterations}[/bold white]")
        ...
        macro_nutrients = ['Carbohydrates (g)', 'Protein (g)', 'Total Fats (g)', 'Alpha-linolenic acid (g)',
                          'Linoleic acid (g)', 'EPA+DHA (mg)', 'Soluble Fiber (g)', 'Insoluble Fiber (g)', 'Water (ml)']
        vitamins = [k for k in all_estimates.keys() if 'Vitamin' in k]
        minerals = ['Calcium (mg)', 'Phosphorus (mg)', 'Magnesium (mg)', 'Potassium (mg)', 'Sodium (mg)',
                   'Chloride (mg)', 'Iron (mg)', 'Zinc (mg)', 'Copper (mcg)', 'Selenium (mcg)',
                   'Manganese (mg)', 'Iodine (mcg)', 'Chromium (mcg)', 'Molybdenum (mcg)']
        amino_acids = [k for k in all_estimates.keys() if k in ['Leucine (g)', 'Lysine (g)', 'Valine (g)',
                       'Isoleucine (g)', 'Threonine (g)', 'Methionine (g)', 'Phenylalanine (g)',
                       'Histidine (g)', 'Tryptophan (g)']]
        other = [k for k in all_estimates.keys() if k not in macro_nutrients + vitamins + minerals + amino_acids]
...
if __name__ == "__main__":
    ...
    if result:
        ...
        key_nutrients = ['Protein (g)', 'Carbohydrates (g)', 'Total Fats (g)',
                        'Vitamin C (mg)', 'Iron (mg)', 'Calcium (mg)']
        ...
        console.print(f"[dim]All {len(result['final_estimates'])} nutrients tracked successfully[/dim]")